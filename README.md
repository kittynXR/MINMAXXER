...
- An optional outgoing-healing-to-others total on the live browser, desktop, and native VR HUDs. Only positive healing events with a different named player as the target count; self-heals and unattributed healing are excluded.
...
- Native audio cues when a fresh `BOSS TARGET?` signal switches to the local player and when a later live ownership transfer moves that same boss to another known player. The urgent warning and softer all-clear are enabled together by default, have independent anti-chatter cooldowns, work while minimized and in headless mode, and can be disabled or routed to any active Windows audio output in Settings.
